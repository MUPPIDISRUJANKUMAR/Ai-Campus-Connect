...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange, isMobileSidebarOpen, onCloseMobileSidebar }) => {
  const { user } = useAuth()
                  ...
                  {menuItems.map((item) => {
                    ...
                    return (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                        onClick={() => { onViewChange(item.id); onCloseMobileSidebar(); }}
                        className={cn(
